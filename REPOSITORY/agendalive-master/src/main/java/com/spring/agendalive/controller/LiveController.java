...
public class LiveController {
	...
	/*
	 * @GetMapping("/lives") public ResponseEntity<Page<LiveDocument>>
	 * getAllLives(@PageableDefault(page = 0, size = 10, sort = "id", direction =
	 * Sort.Direction.ASC) Pageable pageable,
	 * 
	 * @RequestParam(required = false) String flag){ Page<LiveDocument> livePage =
	 * liveService.findAll(pageable, flag); if(livePage.isEmpty()) { return new
	 * ResponseEntity<>(HttpStatus.NOT_FOUND); }else { return new
	 * ResponseEntity<Page<LiveDocument>>(livePage, HttpStatus.OK); } }
	 */
    
    
    @GetMapping("/lives")
    public ResponseEntity<Page<LiveDocument>> getAllLives(String flag){
        Optional<LiveDocument> livePage = liveService.findById( "1");
        if(!livePage.isPresent()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }else {
            return new ResponseEntity<Page<LiveDocument>>(HttpStatus.OK);
        }
    }
    ...
    public ResponseEntity<LiveDocument> saveLive(@RequestBody @Valid LiveDocument live) {
        live.setRegistrationDate(LocalDateTime.now().toString());
